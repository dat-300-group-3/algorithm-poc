...
class StageThree:
    def __init__(self) -> None:
        self.df_freq_potential_fraud_users = pd.read_csv(
            "data/freq_potential_fraud_users.csv"
        )
        self.df_freq_genuine_users = pd.read_csv("data/freq_genuine_users.csv")

        fraud_fit = powerlaw.Fit(
            self.df_freq_potential_fraud_users["frequency"], verbose=False
        )
        genuine_fit = powerlaw.Fit(
            self.df_freq_genuine_users["frequency"], verbose=False
        )

        fraud_alpha = fraud_fit.alpha
        fraud_threshold = fraud_fit.xmin

        genuine_alpha = genuine_fit.alpha
        genuine_threshold = genuine_fit.xmin

        threshold = max(fraud_threshold, genuine_threshold)

        print('fraud threshold:', fraud_threshold)
        print('genuine threshold:', genuine_threshold)

        percentile = (1 - 0.0017195426)
        t = self.df_freq_potential_fraud_users['frequency'].quantile(percentile)
...
class AnomalyDetector:
    ...
    def perform_chi_square(self, t_values_list, is_attributed_values_list):
        for i, column in enumerate(["app", "device", "os", "channel"]):
            ...
            is_attributed_frequencies = [
                is_attributed_values.get(key, 0) for key in all_keys
            ]

            final_filtered_keys = [
                key
                for key, t_freq, exp_freq in zip(
                    all_keys, t_frequencies, is_attributed_frequencies
                )
                if t_freq > exp_freq
            ]
            t_frequencies = [t_values.get(key, 0) for key in final_filtered_keys]
            is_attributed_frequencies = [
                is_attributed_values.get(key, 0) for key in final_filtered_keys
            ]
            ...
            t_frequencies = [
                freq * (total_is_attributed_frequencies / total_t_frequencies)
                for freq in t_frequencies
            ]
            is_attributed_frequencies = [
                max(freq, 1e-10) for freq in is_attributed_frequencies
            ]
            ...
            if p < 0.01:
                ...
                for j, key in enumerate(final_filtered_keys):
                    ...
                    if (diff > self.expected_ratio * is_attributed_frequencies[j]) & (
                        diff > 3
                    ):
                ...
                self.anomalous_values.append(
                    {"column": column, "anomalous_values": column_anomalous_values}
                )
...
class ReportGenerator:
    ...
    def print_anomalies(self):
        for column_values in self.anomalies:
            ...
            for value_info in column_values["anomalous_values"]:
                print(
                    f"Value: {value_info['value']}, Observed: {value_info['observed']}, "
    ...
    def save_anomalies_to_csv(self):
        ...
        with open(anomaly_file_path, "w") as file:
            ...
            for column_values in self.anomalies:
                for value_info in column_values["anomalous_values"]:
                    file.write(
                        f"{column_values['column']},{value_info['value']},"
    ...
    def save_thresholds_to_csv(self):
        ...
        anomaly_thresholds_df.to_csv(
            f"anomalous_thresholds_{self.file_name}", index=False
        )
...
if __name__ == "ggg":
    ...
    alpha_verified, threshold_verified = power_law_analyzer.fit_power_law(
        df_verified["frequency"]
    )
    ...
    anomalous_values = anomaly_detector.perform_chi_square(
        t_values_list, is_attributed_values_list
    )
